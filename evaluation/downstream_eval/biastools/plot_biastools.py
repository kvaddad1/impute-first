...
def plot_balance(balance_delete, balance_SNP, balance_insert, output_name, len_bd, list_incidents, list_plot_name):
    ...
    f, a0 = plt.subplots(1, 1, figsize=(10,5)) #Adjust this value as necessary
    f.set_size_inches(10,5) #Adjust this value as necessary
    ...
    for idx, name in enumerate(list_plot_name):
        a0.errorbar(t, (1-balance_list[idx]), yerr=(balance_1st[idx], balance_3rd[idx]),capsize=3, fmt='-o', color=colors[idx], label=name)
    a0.legend()
    f.subplots_adjust(bottom=0.4)  # Adjust this value as necessary
    a0.legend(loc='upper center', bbox_to_anchor=(0.5, -0.2), ncol=3, frameon=False)
    a0.set_ylim([0.2, 0.7]) #Adjust this value as necessary 
    a0.axhline(y=0.5, color='gray', linestyle='dashdot', linewidth=0.9)
    a0.set(ylabel='Fraction of alternate allele')
    a0.set(xlabel='Insertion (+) or deletion (-) length')
    a0.set_title("Bias-by-allele-length plot", fontsize=14)
    addlabels(t, list_incidents, len_bd)
    plt.savefig(output_name + '.indel_balance.pdf',dpi=300)
